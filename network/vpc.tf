...
resource "aws_subnet" "public_subnet" {
  ...
  #availability_zone = "${data.aws_availability_zones.available.all.names[count.index]}"
 availability_zone       = "${data.aws_availability_zones.all.names[count.index]}"
  tags = "${
    map(
    "Name", "Public_subnet_${data.aws_availability_zones.all.names[count.index]}",
...
resource "aws_subnet" "private_subnet" {
  ...
  tags = "${
    map(
    "Name", "Private_subnet_${data.aws_availability_zones.all.names[count.index]}",
    "kubernetes.io/cluster/${var.cluster_name}" , "owned",
...
resource "aws_subnet" "master_subnet" {
  ...
  tags = {
    Name = "Master_${element(data.aws_availability_zones.all.names, count.index)}"
  }
}
...
resource "aws_subnet" "worker_subnet" {
  ...
  tags = {
    Name = "Worker_${element(data.aws_availability_zones.all.names, count.index)}"
  }
}
